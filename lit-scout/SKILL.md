...
# lit-scout — AI/ML Literature Scout
...
| Use lit-scout when | Use a different skill when |
|---|---|
| You need to find papers in an area | You already have papers and need to position → `related-positioning` |
| You need verified citations for a draft | You need to check citations in existing draft → `integrity-check` |
| You're building an annotated bibliography | You need to write the related work prose → `related-positioning` |
...
Areas where the literature is thin or contested — feeds `idea-forge` and `related-positioning`.
...
- Annotated bibliography → `related-positioning` (for differentiation analysis)
- Annotated bibliography → `paper-writer` (for citation insertion)
- Gap notes → `idea-forge` (for new directions)
...
- `idea-forge` — for direction generation
- `related-positioning` — for differentiation
- `paper-writer` — to embed citations
- `integrity-check` — to audit citations in a draft
