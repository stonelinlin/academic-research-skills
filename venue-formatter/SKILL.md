...
# venue-formatter — AI Venue Submission Packager
...
| Use venue-formatter when | Use a different skill when |
|---|---|
| You have a draft and need final compilation | You're still drafting → `paper-writer` |
| You need reproducibility / disclosure / broader-impact | You need the experimental method → `method-architect` |
| You want camera-ready help post-acceptance | You want pre-submission integrity → `integrity-check` |
...
5. Include reviewer-requested revisions (if `revision_plan` provided from `rebuttal-coach`)

## IRON RULES

1. **Page limit is hard.** If draft exceeds page limit, formatter REFUSES to submit. User must trim via `paper-writer revise mode`.
...
- `paper-writer` — provides the draft prose
- `figure-smith` — provides the figures
- `integrity-check` — runs before final compile
- `rebuttal-coach` — provides revision_plan for camera-ready
