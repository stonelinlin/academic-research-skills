...
# rebuttal-coach — AI Venue Rebuttal Helper
...
| Use rebuttal-coach when | Use a different skill when |
|---|---|
| You have actual reviewer comments | Pre-submission self-review → `paper-reviewer` |
| You're under venue word/page limits | You want a full revision → `paper-writer revise mode` |
| You need to triage and prioritize | You need to update related work for new reviewer-mentioned papers → `related-positioning` |
...
| 6 | Hand-waving on missing-related-work concerns | Use `related-positioning` to compute proper differentiation |
...
- Issue inventory → `paper-writer` (revision mode) for full revision after rebuttal
- Reviewer-threat patterns → `paper-reviewer` (calibration mode) for next paper
- Missing related work → `related-positioning` for proper coverage
...
- `paper-reviewer` — to anticipate reviewer concerns BEFORE submission
- `paper-writer` (revise mode) — for full revision after rebuttal acceptance
- `related-positioning` — when reviewers ask for missing related work
- `integrity-check` — before submitting rebuttal claims about new experiments
