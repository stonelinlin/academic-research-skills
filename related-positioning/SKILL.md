...
# related-positioning — Differentiate Your Work from Existing Papers
...
| Use related-positioning when | Use a different skill when |
|---|---|
| You know which papers might overlap and need to articulate differences | You don't know which papers to compare → `lit-scout` |
| You're writing the Related Work section | You're writing other sections → `paper-writer` |
| Reviewer asks "how is this different from X" → defending | Need a full review report → `paper-reviewer` |
...
- Positioning matrix → consumable by `paper-writer` (when drafting Related Work)
- Reviewer-threat map → consumable by `paper-reviewer` (predicts review concerns) and `rebuttal-coach` (when rebuttal addresses missing-related-work complaint)
...
- `lit-scout` — finds the candidate papers in the first place
- `paper-writer` — consumes the positioning output to write other sections
- `rebuttal-coach` — uses reviewer-threat map when responding to missing-related-work complaints
- `shared/venue_db/`
