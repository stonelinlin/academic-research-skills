...
# research-pipeline — Optional Full-Workflow Orchestrator

Most users should invoke atomic skills directly (`idea-forge`, `lit-scout`, `paper-writer`, etc.). This meta-skill exists for users who explicitly want a coordinated end-to-end workflow with checkpoints.

## When to Use This (Be Honest)

| Use research-pipeline when | Use atomic skills when |
|---|---|
| You want to be guided through every stage | You know what stage you're at |
| You want enforced integrity gates between stages | You'll invoke integrity-check yourself when needed |
...
|---|---|---|
| 1 | `idea-forge` | No (skip if user has RQ) |
| 2 | `lit-scout` | Yes |
| 3 | `related-positioning` | Yes |
| 4 | `method-architect` | Yes for new work; skip for survey |
| 5 | `integrity-check pre-review` | Yes (after method+early results) |
| 6 | `paper-writer` (multi-section) | Yes |
| 7 | `figure-smith` | Yes |
| 8 | `integrity-check final-check` | Yes |
| 9 | `paper-reviewer` | Yes |
| 10 | `venue-formatter` | Yes |
| post | `rebuttal-coach` | If reviews come back |
...
| Agent | Role | File |
|---|---|---|
| `pipeline_orchestrator_agent` | Stage transitions and checkpoint logic | [`academic-pipeline/agents/pipeline_orchestrator_agent.md`](../academic-pipeline/agents/pipeline_orchestrator_agent.md) |
...
4. **integrity-check at Stage 5 and Stage 8 is non-negotiable.** Block-on-suspect verdict cannot be auto-overridden by orchestrator; user must decide.
...
|---|---|---|
| 1 | Promoting research-pipeline as "the way" | Most users want atomic; surface only on explicit request |
...
- [`academic-pipeline`](../academic-pipeline/) (legacy v3.3) — for the original 10-stage strict pipeline; deprecated but kept for backward compat during 6-month migration
